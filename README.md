...
## ゲームをプレイする場合
  ...
  ※ステージデータを外部に保存するので，管理者権限がなくてもファイルを作成できる場所に配置してください．
...
## 制作意図
  プログラミング面での特徴は以下の２点  
  * キャラクターの追加を簡単に行える  
    * キャラクターはTemplateMethodパターンを用いて設計しているため，ゲームの進行や処理を行うプログラムに変更を加えることなくキャラクターの追加を行うことが可能．  
...
## キャラクターの追加（プログラムの加筆が必要）
  ...
  * 注意事項  
  ...
  3. キャラクターのBodyの属性image_stringは，キャラクター画像のファイル名（拡張子は除く）と一致させること．
...
| 名前	| 画像	| 動き	| 弾	| 備考| 
|:-----------|:------------|:------------|:------------|:------------|
| EnemySmallFace | <img src="https://github.com/SaturnValley/shootingMaker/blob/master/data/images/face.png?raw=true" width="96px"> | 動かない	| 自機狙い弾 | 発射間隔1秒	| HP10画像はEnemyBigFaceの使いまわし| 
